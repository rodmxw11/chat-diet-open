...
Implemented on branch `seamless-food-entry`, phases A-G plus docs cleanup. Two threshold
refinements landed during implementation, both philosophy-consistent:

1. The guarded auto-accept tier is **≥ 0.8** (not only the substring tier's exact 0.8), so a sole
   strong-typo match ("bananna" → banana at ~0.86 edit similarity) auto-logs too - the corpus is
   full of typos. The one-word guard still applies.
2. In a multi-candidate top group at ≥ 0.95, a strictly better score wins outright (a name-exact
   1.0 beats a plural-flip 0.95); the higher-calorie tie-break applies only to *equal* top scores.

Also fixed beyond the plan: the estimate path (`useEstimate`) now checks the exact alias first,
which is what actually minted the duplicate greek-yogurt rows (they were MODEL_ESTIMATE, not UPC
inserts); `log_food_by_upc` shares the scanner's upsert path so typed UPCs can't duplicate either.
